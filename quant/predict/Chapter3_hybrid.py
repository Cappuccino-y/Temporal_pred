...
def train_pro(train_dataloader,dev_dataloader, model, device, loss_fn):
    ...
    for epoch in range(config['epoch_num']):
        ...
        for x, y in train_dataloader:
            ...
            pred = model(x,y,mode="train",epoch=epoch)
        ...
        if train_loss < loss :
            ...
            torch.save(model.state_dict(), './save_paras/{}'.format(config['save_name']))
            early_point = 0
        else:
            early_point += 1
...
def dev_pro(dev_dataloader,model,device,loss_fn):
    ...
    for x,y in dev_dataloader:
        x,y=x.to(device),y.to(device)
        with torch.no_grad():
            pred=model(x,y,mode="test",epoch=0)
...
def test(tt_set, model, device):
    ...
    for x, y in tt_set:  # iterate through the dataloader
        ...
        with torch.no_grad():  # disable gradient calculation
            pred = model(x,y,mode="test",epoch=0)  # forward pass (compute output)
...
# model = STA_LSTM(feature_num=feature_num,feature_num_append=feature_num_append,sa_hidden=sa_hidden,output_size=target_num,length=window_length,temp_node=temp_node,ta_hidden=ta_hidden)
...
model.to(get_device())
loss_train=train_pro(dataloader_train,dataloader_dev,model,get_device(),loss_function)
...
model.load_state_dict(torch.load('./save_paras/{}'.format(config['save_name'])))
model.to(get_device())
