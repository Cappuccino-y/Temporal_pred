...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=2.5):
        ...
        self.fc = nn.Sequential(# Excitation操作的定义
            nn.Linear(channel, reduction_channel, bias=False),# 压缩
            nn.PReLU(),
            nn.Linear(reduction_channel, channel, bias=False),# 恢复
            nn.Sigmoid()# 定义归一化操作
        )
...
class STA_LSTM(nn.Module):
    def __init__(self, feature_num,feature_num_append,sa_hidden, ta_hidden, output_size, length,temp_node):
        ...
        self.net1=TemporalConvNet(self.feature_num,[self.input_dim,self.input_dim*2,self.input_dim*4,self.input_dim*4,self.input_dim*2,self.input_dim],kernel_size=2)
        ...
        self.feature_change=nn.Sequential(nn.Linear(sa_hidden,4*ta_hidden),nn.PReLU(),nn.Linear(4*ta_hidden,2*ta_hidden),nn.PReLU())
