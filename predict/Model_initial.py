...
class TemporalLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, former_seq_len, output_dim, temp_node):
        ...
        self.similarity=[0.93276439, 0.86688184, 0.80612168, 0.7489981, 0.70013303,0.65420817]
        ...
        # 权重初始化
        nn.init.xavier_normal_(self.output_fc[0].weight,nn.init.calculate_gain('sigmoid'))
        self.output_fc[0].bias.data=nn.init.xavier_normal_(self.output_fc[0].bias.unsqueeze(0),nn.init.calculate_gain('sigmoid')).squeeze(0)
        # self.output_fc[0].bias.data.zero_()

        # torch.nn.init.normal_(self.output_fc[0].weight,0,1.0 / math.sqrt(self.hidden_dim))
        # torch.nn.init.normal_(self.output_fc[0].bias,0,1.0 / math.sqrt(self.hidden_dim))

        for index,tyu in enumerate(self.similarity):
            gain=nn.init.calculate_gain('sigmoid')
            self.output_fc[index+1].weight.data=tyu*self.output_fc[0].weight.data+(1-tyu)*nn.init.xavier_normal_(torch.zeros_like(
                self.output_fc[0].weight.data), gain)
            self.output_fc[index+1].bias.data=tyu*self.output_fc[0].bias.data+(1-tyu)*nn.init.xavier_normal_(torch.zeros_like(
                self.output_fc[0].bias.unsqueeze(0)), gain).squeeze()
            # self.output_fc[index+1].bias.data=tyu*self.output_fc[0].bias.data+(1-tyu)*torch.normal(0,1.0 / math.sqrt(self.hidden_dim),
            #     self.output_fc[0].bias.size())

        self.init_weights()

    def init_weights(self):
        stdv = 1.0 / math.sqrt(self.hidden_dim)
        for name,weight in self.state_dict(keep_vars=True).items():
            if name=='Wa' or name=='Ua'or name=='W' or name=='U' or name=='Va' or name=='W_y':
                if name=='Wa' or name=='Ua' or name=='Va':
                    gain=nn.init.calculate_gain('tanh')
                else:
                    gain=nn.init.calculate_gain('sigmoid')
                weight.data[:,0]=nn.init.xavier_normal_(torch.zeros_like(weight.data[:,0]), gain)
                for index,inherit in enumerate(self.similarity):
                    weight.data[:,(index+1)]=inherit*weight.data[:,0]+(1-inherit)*nn.init.xavier_normal_(torch.zeros_like(weight.data[:,0]), gain)
            elif name=='ba' or name=='bias':
                # weight.data.zero_()
                if name=='ba':
                    gain=nn.init.calculate_gain('tanh')
                else:
                    gain=nn.init.calculate_gain('sigmoid')
                weight.data[0]=nn.init.xavier_normal_(torch.zeros_like(weight.data[0]).unsqueeze(0), gain).squeeze(0)
                for index,inherit in enumerate(self.similarity):
                    weight.data[(index+1)]=inherit*weight.data[0]+(1-inherit)*nn.init.xavier_normal_(torch.zeros_like(weight.data[0]).unsqueeze(0), gain).squeeze(0)
    ...
    def forward(self, H,y_0,embedding,Y,mode,epoch):
        ...
        while t < seq_len:
            ...
            if mode=="test":
                ...
                # transmit=y_true[:,t:t+1] if random.random()<max(self.sup-((self.sup-self.inf)/self.epoch_limit)*epoch,self.inf) else y_t_1[t]
...
class STA_LSTM(nn.Module):
    def __init__(self, feature_num,feature_num_append,sa_hidden, ta_hidden, output_size, length,temp_node):
        ...
        self.net1=TemporalConvNet(feature_num,[feature_num,feature_num*2,feature_num,feature_num],kernel_size=2)
        ...
        self.lstm_enbedding=nn.LSTM(input_size=feature_num_append,hidden_size=sa_hidden,num_layers=1,batch_first=True)
        ...
        self.TA = TemporalLSTM(input_dim=feature_num, hidden_dim=ta_hidden, former_seq_len=length, output_dim=output_size, temp_node=temp_node)
