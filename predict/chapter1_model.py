...
class TemporalLSTM(nn.Module):
    ...
    def forward(self, H,y_0,init):
        ...
        LSTM_h_t =init[ :,:self.hidden_dim ]
        LSTM_c_t =init[:,self.hidden_dim:]
...
class STA_LSTM(nn.Module):
    def __init__(self, feature_num,feature_num_append,sa_hidden, ta_hidden, output_size, length,temp_node):
        ...
        self.lstm_enbedding=nn.LSTM(input_size=feature_num,hidden_size=sa_hidden,num_layers=1,batch_first=True)
    ...
    def forward(self, X):
        ...
        embedding,(_,_)=self.lstm_enbedding(X)
        # embedding=self.feature_change(embedding[:,-1])
        # remap=self.net2(remap)
        # hidden_seq, (h_t_0, c_t_0) = self.SA(remap)
        embedding=self.dropout(embedding)
        init=self.feature_change(embedding[:,-1])
        y_pred, _ = self.TA(embedding,X[:,-1,-1:],init)
        return y_pred
