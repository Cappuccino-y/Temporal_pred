...
from LSTM_Model2 import STA_LSTM
from pcgrad_time import PCGrad
import math
...
config = {
    ...
    'early_stop': 50,
    'optimizer': 'Adam',
    'save_name': 'sintering_model_{}2.pth'.format(__file__.split("/")[-1].split(".")[0])
}
...
def train_pro(train_dataloader,dev_dataloader, model, device, loss_fn):
    ...
    early_point = 0
    loss =10000
    optimizer = PCGrad(getattr(torch.optim, config['optimizer'])(model.parameters(), **config['optim_hyper']))
    count = 0
