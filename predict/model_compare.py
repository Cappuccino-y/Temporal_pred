...
from back_up.model_lstm_ori import STA_LSTM
import math
...
class dataset(Dataset):
    def __init__(self,
        ...
        dev_split_start = round(len(input_seq) * 7/9)
        if mode == 'train':
...
model = STA_LSTM(feature_num=feature_num,sa_hidden=sa_hidden,ta_hidden=ta_hidden,output_size=pred_length,length=window_length,spat_node=spat_node,temp_node=temp_node)
model.to(get_device())
