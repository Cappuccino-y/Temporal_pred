...
legend_font = fm.FontProperties(family='Times New Roman', size=10)
Plotdata=[data_arima_pred, data_lstm_pred,  data_LSTNet_pred, data_Autoformer_pred,data1_pred, data2_pred, data3_optimal_pred]
Plotname=['ARIMA','LSTM','LSTNet','Autoformer','TA-LSTM','TSE-TALSTM','TSE-TSBD']
...
for index,(name,pred) in enumerate(Plotcomb):
    ...
    axs[index].legend(['Groud truth','Predicted values'],  prop=legend_font,frameon=False)
    ...
    axs[index].text(300-1.8*len('Prediction results with {}'.format(name)),97 , 'Prediction results with {}'.format(name), fontsize=12,
